...
class InvoiceDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Obtenemos el estado del BLoC para acceder a los datos brutos
    final summaryState = BlocProvider.of<SummaryBloc>(context).state;

    final List<InvoiceItem> items = summaryState.allInvoiceItems
        .where(
          (item) =>
    ...
    final List<Product> allProducts = summaryState.allProducts;
        ...
        children: [
          ...
          _buildDetailCard(
            ...
            children: [
              _buildDetailRow(
                'Total Neto:',
                formatNumber(invoice.amount, deviceLocale),
              ),
              _buildDetailRow(
                'Impuestos:',
                formatNumber(invoice.amounttax, deviceLocale),
              ),
              _buildDetailRow(
                'Total General:',
                formatNumber(
                  (invoice.amount + invoice.amounttax),
                  deviceLocale,
                ),
              ),
              const Divider(),
              _buildDetailRow(
                'A Credito:',
                formatNumber(invoice.credit, deviceLocale),
              ),
              _buildDetailRow(
                'De Contado:',
                formatNumber(invoice.cash, deviceLocale),
              ),
            ],
          ),
          const SizedBox(height: 16),
          _buildDetailCard(
            ...
            children: items.expand((item) {
              ...
              return [
                ...
                _buildDetailRow(
                  'Precio de venta:',
                  formatNumber(item.price, deviceLocale),
                ),
                _buildDetailRow(
                  'Impuesto en venta:',
                  formatNumber(item.tax, deviceLocale),
                ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 8),
          Flexible(flex: 3, child: Text(value, textAlign: TextAlign.end)),
        ],
      ),
    );
  }
